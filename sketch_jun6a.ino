...
#define PIN_TFT_DC 17  // LCD 第 8 腳：假設是 DC / A0 / RS
...
// 常見 joystick 模組：往上時 Y 會變小，所以預設反轉 Y
...
void initTracks() {
  ...
  // Demo pattern，方便你一開機就能聽到聲音。
  // 不想要 demo pattern 的話，把這段刪掉即可。
...
void advanceStepFromAudio(uint32_t samplesPerStep) {
  ...
  // Record armed：等 loop 回到 step 0 才開始錄
  ...
  // Recording：每個 step 讀一次目前 joystick 方向
...
void handleDirEvent(Dir4 dir) {
  ...
  // REC_ARMED / RECORDING 狀態下，方向鍵拿來輸入音符，不做選單操作。
